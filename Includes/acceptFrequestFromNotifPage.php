<?php
  ...
  if(isset($_SESSION['userId'])){
    ...
    //selecting friend id from frequest
    $sql_sel0 = "SELECT * FROM tbl_frequests WHERE idReq = ?";
    ...
    if(!mysqli_stmt_prepare($stmt_sel0, $sql_sel0)){
        ...
        mysqli_stmt_bind_param($stmt_sel0, 'i', $idFreq);
        mysqli_stmt_execute($stmt_sel0);
        $res0 = mysqli_stmt_get_result($stmt_sel0);
        $row0 = mysqli_fetch_assoc($res0);
        $idFriend = $row0['idUser'];
    }
    $zero = 0;
